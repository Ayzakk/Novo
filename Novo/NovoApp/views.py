...
def home_view(request):
    """Vista principal a la que se redirige el usuario después de iniciar sesión."""
    return render(request, 'home.html')

def laptops_view(request):
    # Obtener todas las laptops de la base de datos
    laptops = Laptops.objects.all()
    # Pasar las laptops al HTML
    return render(request, 'laptops.html', {'laptops': laptops})

def logout_view(request):
    logout(request)  # Cierra la sesión del usuario
    return redirect('login')  # Redirige a la página de login o la que desees